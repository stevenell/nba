...
def update_live():
    ...
    for g in scoreboard.ScoreBoard().get_dict()['scoreboard']['games']:
         live_game_dict[g['gameId']]= {k:v for k,v in g.items() if k in ['period','gameClock','homeTeam','awayTeam','periods']}
...
def get_game_df(game_id,full=False):
    ...
    if(pbp_avail):
        ...
        if full:
            ...
            pbp = pbp_full.loc[((pbp_full['EVENTMSGTYPE']==12)&(pbp_full['SCORE']=='0 - 0')) | (pbp_full['EVENTMSGTYPE']==13),:].copy()
        ...
        pbp['prob'] = pbp.apply(lambda x: get_win_prob(x['away_score'],x['home_score'],x['PERIOD'],x['PCTIMESTRING']),axis=1)
    ...
    return(pbp.rename(columns={"PERIOD": "period", "PCTIMESTRING": "game_clock", "EVENTNUM": "play_id"})[['away_score','home_score','game_clock','period','prob','prob_diff','prob_category','description','play_id']])
...
def get_colors(home_id,away_id):
    if random.random() > 0.5:
        ...
        home = team_dict[home_id]['color_2']
    
    if color_distance(home,team_dict[away_id]['color_1']) > color_distance(home,team_dict[away_id]['color_2']):
        away = team_dict[away_id]['color_1']
    else:
        away = team_dict[away_id]['color_2']
...
def get_video_url(game_id,event_id):
    try:
        url = 'https://stats.nba.com/stats/videoeventsasset?GameEventID={event}&GameID={game}'.format(
            event = event_id,
            game = game_id
        )
        ...
        json_set = r.json()
        return(json_set['resultSets']['Meta']['videoUrls'][0]['lurl'])
    except:
        return(None)
...
#    dcc.Store(id='game-date'),
    ...
    html.Label('Click on a Game for More Info!'),
        dash_table.DataTable(id='game-list',
             columns = [
                ...
                {'id':'prob','name':'Probability of Home Win','type':'numeric','format':dash_table.FormatTemplate.percentage(1)}
        ])
    ]),
    html.Hr(),
    html.Div([
    ...
    html.Div([
        ...
        html.Label('Win Probability Graph. Click for Highlights!'),
...
@app.callback(
    ...
    [Input('win-prob-graph', 'clickData'),
    Input('dates-dropdown', 'date')]
)
def display_click_data(clickData,game_date):
    
    if clickData is not None:
        play_id = clickData['points'][0]['customdata'][3]
        game_id = clickData['points'][0]['customdata'][4]
        return(get_video_url(game_id,play_id))
    else:
        return None
